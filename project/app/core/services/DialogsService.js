...
angular.module("services.dialogs-service", []).factory("DialogsService", DialogsService);
DialogsService.$inject = ["$rootScope", "$window"];


function DialogsService($rootScope, $window) {
    ...
    return {
        ...
        prompt: function (title, message, button_1, button_2, callback) {
            if ($rootScope.env === 'DEV' || $rootScope.env === 'HEROKU') {
                ...
                navigator.notification.confirm(message, function (confirmStatus) {
